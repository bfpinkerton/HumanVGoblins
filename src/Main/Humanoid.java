    ...
    public void combat(Humanoid defender) {
        ...
        try{
            ...
            while (defender.getHealth() > 0 && this.getHealth() > 0) {
                ...
                int thisAttack = 10 * (RandomRan.generator(1, 6) * ((this.getAttack() / defender.getDefense()) * this.getLuck()));
                int defAttack = 10 * (RandomRan.generator(1, 6) * ((defender.getAttack() / this.getDefense()) * defender.getLuck()));

                if (thisAttack <= 0){
                    thisAttack = RandomRan.generator(2, 4) * (10 * this.getLuck());
                }
                if (defAttack <= 0){
                    defAttack = RandomRan.generator(2, 4) * (10 * defender.getLuck());
                }
                if(this.getHealth() > 0){
